...
export async function GET(request: Request) {
  ...
  if (url.searchParams.get("summary") === "1") {
    const [{ data: rows, error }, { data: reads }] = await Promise.all([
      auth.admin.from("core_events").select("id,details,actor_id,metadata,created_at").eq("event_type", "family.chat_message").order("created_at", { ascending: false }).limit(250),
      auth.admin.from("core_events").select("metadata,created_at").eq("event_type", "family.chat_read").eq("actor_id", auth.member.id).order("created_at", { ascending: false }),
    ]);
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
    const lastRead = new Map<string, number>();
    for (const read of reads ?? []) { const conversation = String(read.metadata?.conversationId ?? ""); if (!lastRead.has(conversation)) lastRead.set(conversation, new Date(read.metadata?.readAt ?? read.created_at).getTime()); }
    const visible = (rows ?? []).filter((row) => row.actor_id !== auth.member.id && mayAccess(auth.member.id, String(row.metadata?.conversationId ?? "")));
    ...
    for (const row of visible) { const conversation = String(row.metadata?.conversationId ?? ""); if (new Date(row.created_at).getTime() > (lastRead.get(conversation) ?? 0)) unreadByConversation[conversation] = (unreadByConversation[conversation] ?? 0) + 1; }
    return NextResponse.json({ messages: recent, unreadByConversation, totalUnread: Object.values(unreadByConversation).reduce((sum, count) => sum + count, 0) });
  }
  ...
  const [{ data: members }, { data: rows, error }, { data: statusRows }, { data: allRows }, { data: readRows }] = await Promise.all([
    ...
    auth.admin.from("core_events").select("actor_id,metadata,created_at").eq("event_type", "family.chat_message").order("created_at", { ascending: false }).limit(500),
    auth.admin.from("core_events").select("metadata,created_at").eq("event_type", "family.chat_read").eq("actor_id", auth.member.id).order("created_at", { ascending: false }),
  ]);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  ...
  for (const row of allRows ?? []) { const id = String(row.metadata?.conversationId ?? ""); if (row.actor_id !== auth.member.id && mayAccess(auth.member.id, id) && new Date(row.created_at).getTime() > (readMap.get(id) ?? 0)) unreadCounts[id] = (unreadCounts[id] ?? 0) + 1; }
  const conversations = [
...
export async function PATCH(request: Request) {
  ...
  if (!mayAccess(auth.member.id, conversationId)) return NextResponse.json({ error: "لا تملك صلاحية فتح هذه المحادثة." }, { status: 403 });
  const { data: existing } = await auth.admin.from("core_events").select("id").eq("event_type", "family.chat_read").eq("actor_id", auth.member.id).eq("metadata->>conversationId", conversationId).order("created_at", { ascending: false }).limit(1).maybeSingle();
  const metadata = { conversationId, readAt: new Date().toISOString() };
  const result = existing ? await auth.admin.from("core_events").update({ metadata }).eq("id", existing.id) : await auth.admin.from("core_events").insert({ event_type: "family.chat_read", title: "قراءة المحادثة", actor_id: auth.member.id, metadata });
