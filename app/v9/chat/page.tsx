...
export default function FamilyChatPage() {
  ...
  return <main dir="rtl" className="min-h-screen bg-[#07100d] p-0 text-white sm:p-5"><div className="mx-auto flex h-[100dvh] max-w-7xl overflow-hidden border-white/10 bg-[#0b1411] sm:h-[calc(100dvh-40px)] sm:rounded-[28px] sm:border">
    ...
    <section className={`${mobileChat ? "flex" : "hidden"} min-w-0 flex-1 flex-col bg-[#07100d] md:flex`}>
      ...
      <form onSubmit={send} className="relative flex items-end gap-2 border-t border-white/10 bg-[#17241f] p-3"><input ref={photoInput} type="file" accept="image/jpeg,image/png,image/webp,image/gif" onChange={(event) => void upload(event.target.files?.[0])} className="hidden" /><input ref={videoInput} type="file" accept="video/mp4,video/quicktime" onChange={(event) => void upload(event.target.files?.[0])} className="hidden" /><input ref={audioInput} type="file" accept="audio/*" onChange={(event) => void upload(event.target.files?.[0])} className="hidden" />{attachmentsOpen && <div className="absolute bottom-16 right-3 grid gap-2 rounded-2xl border border-white/10 bg-[#202c27] p-2 shadow-2xl"><button type="button" onClick={() => photoInput.current?.click()} className="flex items-center gap-3 rounded-xl px-4 py-3 text-sm hover:bg-white/5"><span className="text-xl">🖼️</span>إرسال صورة</button><button type="button" onClick={() => videoInput.current?.click()} className="flex items-center gap-3 rounded-xl px-4 py-3 text-sm hover:bg-white/5"><span className="text-xl">🎬</span>إرسال فيديو</button><button type="button" onClick={() => audioInput.current?.click()} className="flex items-center gap-3 rounded-xl px-4 py-3 text-sm hover:bg-white/5"><span className="text-xl">🎵</span>إرسال ملف صوتي</button></div>}<button type="button" onClick={() => setAttachmentsOpen((value) => !value)} className={`grid h-11 w-11 shrink-0 place-items-center rounded-full text-xl transition ${attachmentsOpen ? "rotate-45 bg-emerald-500" : "bg-white/5"}`}>＋</button><textarea value={text} onChange={(event) => setText(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); event.currentTarget.form?.requestSubmit(); } }} placeholder={sending ? "جاري رفع الملف…" : "اكتب رسالة…"} disabled={sending} rows={1} className="max-h-28 min-h-11 flex-1 resize-none rounded-2xl border border-white/5 bg-[#202c27] px-4 py-3 text-sm outline-none focus:border-emerald-300/30 disabled:opacity-50" /><button disabled={sending || !text.trim()} className="grid h-11 w-11 shrink-0 place-items-center rounded-full bg-emerald-500 text-xl text-white disabled:opacity-40">➤</button></form>
    </section>
